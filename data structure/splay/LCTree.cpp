...
//		Node() : l(nullptr),r(nullptr),sz(0),v(0),sm(0){}						//単位元!!
		...
		void splay(){			//thisを"そのsplayの"rootにもってくる
			while(true){
				int ps=pos();
				if(ps==0) break;
				int pps=p->pos();
		...
		NP expose(){			//thisを"LCの"rootにもってくる
			NP now=this,prev=last;
			do{
				now->splay();
				now->r=prev;
				now->update();
		...
		void cut(){			//thisとthisのactual treeでのparentをcut
			expose();
			l->p=nullptr;
			l=nil;
			update();
