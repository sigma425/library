...
struct segtree{
	...
	int count(int a,int b,int ya,int yb,int l,int r,int k){		//[a,b) * [ya,yb)
		if(b<=l||r<=a) return 0;
		if(a<=l&&r<=b) return seg[k].lower_bound(yb);
		return count(a,b,ya,yb,l,(l+r)/2,k*2)+count(a,b,ya,yb,(l+r)/2,r,k*2+1);
	}
	int count(int a,int b){
//		printf("[0,%d]*[0,%d]\n",a,b);
		return count(0,a+1,0,b+1,0,N,1);
	}
}DL,DR,UL,UR;
...
int main(){
	...
	ll sum=0;
	cin>>W>>H>>N;
	L.init(W+1),R.init(W+1),D.init(H+1),U.init(H+1),DL.init(W+1),DR.init(W+1),UL.init(W+1),UR.init(W+1);
	rep(tt,N){
		int xa,ya,xb,yb;
		cin>>xa>>ya>>xb>>yb;
		xa--,ya--;
		ll ans=(ll)(xb-xa)*(yb-ya);
		int c1=L.sum(xa+1)+R.sum(W-xb+1)+D.sum(ya+1)+U.sum(H-yb+1);
		int c2=DL.count(xa,ya)+DR.count(W-xb,ya)+UL.count(xa,H-yb)+UR.count(W-xb,H-yb);
		if(c1-c2==cnt){
			L.add(xb),R.add(W-xa),D.add(yb),U.add(H-ya);
			DL.add(xb,yb),DR.add(W-xa,yb),UL.add(xb,H-ya),UR.add(W-xa,H-ya);
			cnt++;
			sum+=ans;
		}
		cout<<sum<<endl;
